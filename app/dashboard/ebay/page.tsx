...
import DescrizioneTemplatePanel from './DescrizioneTemplatePanel';
...
export default async function EbayConfigPage({
  ...
  const [connection, demolitore] = await Promise.all([
    ...
    prisma.demolitore.findUnique({
      where: { id: session.id },
      select: { email: true, descrizioneTemplate: true },
    }),
  ]);
    ...
    <DashboardShell ragioneSociale={session.ragioneSociale} email={demolitore?.email ?? session.email}>
      <div className="max-w-7xl mx-auto p-4 sm:p-8">
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <DescrizioneTemplatePanel initial={demolitore?.descrizioneTemplate ?? null} />
