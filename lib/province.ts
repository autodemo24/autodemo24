// Lista province italiane con sigla 2 lettere ISO 3166-2:IT
// La sigla è quella salvata in DB. Il nome è solo per UI.
...
const NAME_BY_CODE: Record<string, string> = Object.fromEntries(PROVINCE.map((p) => [p.code, p.name]));

// Ritorna il nome esteso dato il codice 2 lettere. Se input è già un nome, lo ritorna così com'è.
export function provinciaNome(codeOrName: string): string {
  if (!codeOrName) return '';
  const upper = codeOrName.trim().toUpperCase();
  if (upper.length === 2 && NAME_BY_CODE[upper]) return NAME_BY_CODE[upper];
  return codeOrName;
}
